...
# ... (Las clases de Dino, ArkDrop, Tame, Craft, Imprint, Alpha, Pokemon van aquí)
# Voy a poner el código COMPLETO con las traducciones.
    ...
    async def feed(self, interaction: discord.Interaction, food: str):
        ...
        try:
            if food == self.correct_food:
                uid = str(interaction.user.id)
                config.points_data[uid] = config.points_data.get(uid, 0) + 200
                await interaction.response.send_message(f"🦕 **TAMED!** {interaction.user.mention} tamed the {self.dino_name} (+200 pts).", ephemeral=False)
    ...
    async def check_mat(self, interaction: discord.Interaction, mat_clicked: str):
        ...
        try:
            if mat_clicked == self.correct_mat:
                uid = str(interaction.user.id)
                config.points_data[uid] = config.points_data.get(uid, 0) + 200
                await interaction.response.send_message(f"🔨 **Correct!** {interaction.user.mention} crafted it (+200 pts).", ephemeral=False)
...
class Minigames(commands.Cog):
    ...
    async def show_recipes(self, ctx):
        embed = discord.Embed(title=f"{config.EMOJI_BLOOD} **HELL RECIPES**", color=0x990000)
    ...
    async def dino_game_loop(self):
        ...
        if not channel: 
            print(f"[ERROR] Dino Channel ID {config.DINO_CHANNEL_ID} not found.")
            return
    ...
    async def minigames_auto_loop(self):
        ...
        if not channel: 
            print(f"[ERROR] Minigames Channel ID {config.MINIGAMES_CHANNEL_ID} not found.")
            return
    ...
    async def spawn_game(self, channel):
        ...
        if game_type == 1:
            embed = discord.Embed(title="RED SUPPLY DROP!", description="High-level loot!", color=discord.Color.red())
            ...
            embed = discord.Embed(title="Unconscious Dino!", description=f"What does **{data['name']}** eat?", color=discord.Color.green())
            embed.set_image(url=data["url"])
            return await channel.send(embed=embed, view=ArkTameView(data["food"], data["name"]))
        elif game_type == 3:
            data = random.choice(config.DATA_CRAFTING)
            embed = discord.Embed(title="Crafting Table", description=f"Material for **{data['name']}**?", color=discord.Color.orange())
            embed.set_image(url=data["url"])
            return await channel.send(embed=embed, view=ArkCraftView(data["mat"]))
        elif game_type == 4:
            embed = discord.Embed(title="Baby Rearing", description="Baby is crying. **Guess care.**", color=discord.Color.purple())
            ...
            embed = discord.Embed(title=f"⚠️ {data['name'].upper()} DETECTED", description=f"Risk it?\nReward: +{data['win']} | Risk: -{data['loss']}", color=data['color'])
            embed.set_image(url=data["url"])
            ...
            embed = discord.Embed(title="Who's that Pokémon?", description=f"Type of **{data['name']}**?", color=discord.Color.gold())
            embed.set_image(url=data["url"])
    ...
    async def before_loops2(self): await self.bot.wait_until_ready()
