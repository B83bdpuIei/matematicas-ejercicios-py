...
class WipeConfigModal(discord.ui.Modal, title="⚙️ CONFIGURE NEXT WIPE"):
    ...
    time_input = discord.ui.TextInput(label="Time (HH:MM) - Spain Time", placeholder="e.g. 17:00", required=True, min_length=5, max_length=5)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if not hasattr(config, 'wipes_data'): config.wipes_data = {}
            
            config.wipes_data["next"] = date_str
            config.wipes_data["next_timestamp"] = ts
            
            # Update Voice Channels
            try:
                next_ch = interaction.guild.get_channel(config.NEXT_WIPE_CHANNEL_ID)
                if next_ch: await next_ch.edit(name=f"💀 NEXT WIPE: {date_str}")
            except: pass
            
            embed = discord.Embed(title="✅ WIPE CONFIGURED", color=0x00FF00)
            embed.description = f"**Next Wipe:** {date_str} at {time_str}\n**Timestamp:** <t:{ts}:R>"
...
class WipeControlView(discord.ui.View):
    ...
    @discord.ui.button(label="📅 SET NEXT WIPE", style=discord.ButtonStyle.primary, emoji="⚙️")
    ...
    async def force_update(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            l_txt = f"🩸 LAST WIPE: {config.wipes_data.get('last', 'Unknown')}"
            n_txt = f"💀 NEXT WIPE: {config.wipes_data.get('next', '¿?')}" if config.wipes_data.get('next') else "💀 NEXT WIPE: ¿?"
            
            c1 = interaction.guild.get_channel(config.LAST_WIPE_CHANNEL_ID)
            if c1: await c1.edit(name=l_txt)
            
            c2 = interaction.guild.get_channel(config.NEXT_WIPE_CHANNEL_ID)
            if c2: await c2.edit(name=n_txt)
            
            await interaction.followup.send("✅ Voice Channels Updated!", ephemeral=True)
...
class VaultModal(discord.ui.Modal, title="🔐 SECURITY OVERRIDE"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if self.code_input.value == config.vault_state["code"]:
            ...
            embed.description = (f"{config.EMOJI_VAULT_WINNER_CROWN} **WINNER:** {interaction.user.mention}\n{config.EMOJI_VAULT_CODE_ICON} **CODE:** `{config.vault_state['code']}`\n{config.EMOJI_GIFT_NEW} **LOOT:** {config.vault_state['prize']}\n{config.EMOJI_POINTS} **BONUS:** +2000")
...
class Systems(commands.Cog):
    ...
    async def wipe_monitor(self):
        if not config.wipes_data.get("next") or not config.wipes_data.get("next_timestamp"):
            return

        now_ts = int(time.time())
        target_ts = config.wipes_data["next_timestamp"]

        if now_ts >= target_ts:
            print("[WIPE SYSTEM] ⚠️ WIPE TIME REACHED! Switching cycles...")
            config.wipes_data["last"] = config.wipes_data["next"]
            config.wipes_data["next"] = None
            config.wipes_data["next_timestamp"] = 0
            
            try:
                # Usar el primer servidor donde esté el bot
                if self.bot.guilds:
                    guild = self.bot.guilds[0]
                    c_last = guild.get_channel(config.LAST_WIPE_CHANNEL_ID)
                    if c_last: await c_last.edit(name=f"🩸 LAST WIPE: {config.wipes_data['last']}")
                    c_next = guild.get_channel(config.NEXT_WIPE_CHANNEL_ID)
                    if c_next: await c_next.edit(name="💀 NEXT WIPE: ¿?")
            except Exception as e:
                print(f"[WIPE SYSTEM ERROR] {e}")

    @wipe_monitor.before_loop
    async def before_wipe_monitor(self):
        await self.bot.wait_until_ready()

    # --- WIPE COMMANDS ---
    @app_commands.command(name="config_wipe", description="ADMIN: Open Wipe Management Menu")
    async def config_wipe(self, interaction: discord.Interaction):
        if not interaction.user.guild_permissions.administrator: return
        embed = discord.Embed(title="⚙️ **WIPE CONTROL PANEL**", color=0x2b2d31)
        embed.description = (
            f"**Current Data:**\n"
            f"🩸 **Last:** `{config.wipes_data.get('last', 'Unknown')}`\n"
            f"💀 **Next:** `{config.wipes_data.get('next', 'Not Set')}`\n\n"
            "Use the buttons below to configure."
        )
        await interaction.response.send_message(embed=embed, view=WipeControlView(), ephemeral=True)

    @commands.command(name="wipe")
    async def wipe_command(self, ctx):
        last_w = config.wipes_data.get("last", "Unknown")
        next_w = config.wipes_data.get("next", None)
        next_ts = config.wipes_data.get("next_timestamp", 0)

        embed = discord.Embed(title="🔥 **HELL CHRONICLES: WIPE SCHEDULE**", color=0x990000)
        embed.add_field(name=f"{config.HELL_ARROW} **LAST WIPE**", value=f"📅 `{last_w}`\n*Complete Wipe*", inline=False)
        
        if next_w and next_ts > 0:
            embed.add_field(name=f"{config.HELL_ARROW} **NEXT WIPE**", value=f"📅 `{next_w}`\n{config.EMOJI_CLOCK_NEW} <t:{next_ts}:R> (Remaining)\n{config.EMOJI_CLOCK_NEW} <t:{next_ts}:F> (Local Time)", inline=False)
        else:
            embed.add_field(name=f"{config.HELL_ARROW} **NEXT WIPE**", value=f"❓ **TO BE ANNOUNCED**\nCheck <#{config.ROLES_CHANNEL_ID}> for info.", inline=False)
            
        embed.set_footer(text="Hell System • Wipe Schedule")
        ...
        try:
            ...
            if channel:
                ...
                for msg_id, gdata in list(config.giveaways_data.items()):
                    self.bot.loop.create_task(self.run_giveaway_timer(int(gdata["channel_id"]), int(msg_id), gdata["end_time"], gdata["prize"], gdata["winners"]))
    ...
    async def start_giveaway(self, interaction: discord.Interaction, time_str: str, prize: str, winners: int = 1):
        ...
        embed = discord.Embed(title=f"{config.EMOJI_FIRE_ANIM} HELL SPONSOR GIVEAWAY {config.EMOJI_FIRE_ANIM}" if is_sponsor else f"{config.EMOJI_PARTY_NEW} GIVEAWAY", color=0x990000 if is_sponsor else 0x00FF00)
        end_ts = int(time.time() + seconds)
        warning_text = f"\n{config.EMOJI_WARN} **ANTI-CHEAT ACTIVE: Remove name tag = Auto-Kick**" if is_sponsor else ""
        embed.description = (f"{config.EMOJI_GIFT_NEW} **Prize:** {prize}\n{config.EMOJI_CLOCK_NEW} **Ends:** <t:{end_ts}:R>\n{config.EMOJI_VAULT_WINNER_CROWN} **Winners:** {winners}\n{warning_text}\nReact with {config.EMOJI_PARTY_NEW} to enter!")
    ...
    async def start_bulk_giveaway(self, interaction: discord.Interaction, time_str: str, prizes_list: str, winners_per_giveaway: int = 1):
        ...
        warning_text = f"\n{config.EMOJI_WARN} **ANTI-CHEAT ACTIVE: Remove name tag = Auto-Kick**" if is_sponsor else ""
        for prize in prizes:
            embed = discord.Embed(title=f"{config.EMOJI_FIRE_ANIM} HELL SPONSOR GIVEAWAY" if is_sponsor else f"{config.EMOJI_PARTY_NEW} GIVEAWAY", color=0x990000 if is_sponsor else 0x00FF00)
            embed.description = (f"{config.EMOJI_GIFT_NEW} **Prize:** {prize}\n{config.EMOJI_CLOCK_NEW} **Ends:** <t:{end_ts}:R>\n{config.EMOJI_VAULT_WINNER_CROWN} **Winners:** {winners_per_giveaway}\n{warning_text}\nReact with {config.EMOJI_PARTY_NEW} to enter!")
            embed.set_footer(text="Hell System • Giveaway")
            msg = await interaction.channel.send(embed=embed)
            await msg.add_reaction(config.EMOJI_PARTY_NEW)
            config.giveaways_data[str(msg.id)] = {"channel_id": interaction.channel_id, "end_time": end_ts, "prize": prize, "winners": winners_per_giveaway}
    ...
    @app_commands.command(name="add_points", description="ADMIN: Add points")
    ...
    @app_commands.command(name="remove_points", description="ADMIN: Remove points")
